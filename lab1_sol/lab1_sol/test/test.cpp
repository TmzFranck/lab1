...
TEST_F(LabTest, testAuthors_fabian_war_krank) {
	...
	const auto message_1_e = fmt::format("1e) wurde bearbeitet von: {}\n", AUTHORS_ONE_E);
	const auto message_1_f = fmt::format("1f) wurde bearbeitet von: {}\n", AUTHORS_ONE_F);
	...
	std::cout << message_1_e;
	std::cout << message_1_f;
...
#else
	std::cout << "AUTHORS_THREE_C war nicht vorhanden. Dies ist ok\n";
#endif
#ifdef AUTHORS_THREE_D
	std::cout << message_3_d;
#else
	std::cout << "AUTHORS_THREE_D war nicht vorhanden. Dies ist ok\n";
#endif
#ifdef AUTHORS_THREE_E
	std::cout << message_3_e;
#else
	std::cout << "AUTHORS_THREE_E war nicht vorhanden. Dies ist ok\n";
#endif
	...
	const auto authors_1_e = std::string(AUTHORS_ONE_E);
	const auto authors_1_f = std::string(AUTHORS_ONE_F);
	...
	ASSERT_GT(authors_1_e.length(), 0);
	ASSERT_GT(authors_1_f.length(), 0);
