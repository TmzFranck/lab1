...
BitmapImage FES::encrypt_fast(const BitmapImage& original_image, const FES::key_type& encryption_key) {
	const auto height = original_image.get_height();
	const auto width = original_image.get_width();
	...
	for (auto y = std::uint32_t(0); y < height; y += 3) {
		for (auto x = std::uint32_t(0); x < width; x += 48) {
			...
			const auto& encrypted_block = encrypt_block_fast(current_block, previous_block);
...
FES::block_type FES::encrypt_block_fast(const block_type& current_block, const block_type& previous_block) {
	auto encrypted_block = block_type{};
	...
	for (auto y = std::size_t(0); y < 3; y++) {
		for (auto x = std::size_t(0); x < 3; x++) {
			...
			if (x > 0) {
				working_row = combine_rows(working_row, current_block[y][x - 1]);
				working_row = combine_rows(working_row, previous_block[y][x - 1]);
			}
			else if (y > 0) {
				working_row = combine_rows(working_row, current_block[y - 1][2]);
				working_row = combine_rows(working_row, previous_block[y - 1][2]);
			}
